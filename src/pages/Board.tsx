          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem' }} className="board-grid">
            {boardMembers.map((m, i) => (
              ...
              >
                ...
                <div style={{ paddingTop: '1rem', borderTop: '1.5px solid #F4F1E9' }}>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                    {m.expertise.map((tag, j) => (
                      <span key={j} style={{ fontFamily: 'Nunito, sans-serif', fontSize: '0.72rem', fontWeight: 700, color: '#5E8A8C', background: '#F4F1E9', padding: '0.25rem 0.7rem', borderRadius: 50, border: '1px solid rgba(94,138,140,0.2)' }}>{tag}</span>
                    ))}
                  </div>
                </div>
              </div>
